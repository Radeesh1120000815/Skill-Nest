...
const ForgotPassword = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6">
      ...
      <div className="absolute top-0 right-0 -mt-10 -mr-10 w-64 h-64 bg-indigo-200 opacity-20 rounded-full blur-3xl"></div>
      <div className="absolute bottom-0 left-0 -mb-10 -ml-10 w-64 h-64 bg-purple-300 opacity-20 rounded-full blur-3xl"></div>
      ...
      <div className="relative z-10 max-w-md w-full bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-2xl p-10 border border-white transition-all">
        ...
        <form onSubmit={handleForgot} className="space-y-6">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className={`w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold text-lg shadow-lg shadow-indigo-200 hover:shadow-xl hover:-translate-y-0.5 transition-all active:scale-95 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
          >
            {loading ? (
