...
export default function Register() {
  useEffect(() => {
    document.title = 'Create Account — Skill Nest';
  }, []);
  ...
  const validate = () => {
    ...
    if (role === "Student") {
      if (!form.id.trim()) {
        ...
        err.id = "Student ID must be in format IT21234567.";
        valid = false;
      }
    } else if (role === "Lecturer") {
      if (!form.id.trim()) {
        err.id = "Lecturer ID is required.";
        valid = false;
      } else if (!/^LE\d{4}$/i.test(form.id)) {
        err.id = "Lecturer ID must be in format LE0012.";
        valid = false;
      }
    }
    if (!form.email.trim()) {
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-100 via-indigo-50 to-purple-50">
      <nav className="bg-slate-900 border-b border-white/10 px-6 flex items-center justify-between h-16 shadow-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <a className="hidden sm:inline nav-a text-white/60 hover:text-white px-3 py-1 rounded transition" href="#">Resources</a>
          <a className="hidden sm:inline nav-a text-white/60 hover:text-white px-3 py-1 rounded transition" href="#">Sessions</a>
      ...
      <main className="flex-1 flex items-center justify-center py-10 px-2">
        <div className="reg-card bg-white rounded-3xl p-8 max-w-lg w-full shadow-xl animate-fadeUp">
          ...
          <p className="text-sm text-slate-500 text-center mb-7">Or <a href="/signin" className="text-blue-700 font-semibold hover:underline">sign in to your existing account</a></p>
          {/* Role selector */}
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              {['Student','Lecturer','Admin'].map((r) => (
                <button type="button" key={r} onClick={() => handleRole(r)} className={`role-tab flex flex-col items-center gap-2 px-3 py-4 rounded-xl border-2 font-semibold text-xs transition ${role===r?'border-blue-700 bg-blue-50 text-blue-700 shadow':'border-slate-200 bg-slate-50 text-slate-400'}`}>{r}</button>
              ))}
            </div>
          </div>
          {/* Email form */}
            ...
            <div className="fields-group border border-slate-200 rounded-xl overflow-hidden mb-5">
              ...
              {(role==="Student"||role==="Lecturer") && <>
                <div className="field-wrap border-b border-slate-200">
                  <input type="text" name="id" value={form.id} onChange={handleChange} placeholder={role==="Student"?"Student ID (e.g. IT21234567)":"Lecturer ID (e.g. LE0012)"} className="block w-full px-6 py-5 text-lg text-slate-900 bg-white outline-none" />
                </div>
      ...
      {/* Footer is now handled by the Footer component below */}
      <Footer />
    </div>
  );
}
